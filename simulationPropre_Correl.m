...
for inoc = 1:length(inocula)  %loop over the inoculum
    for i = 1: Ndrop %loop over the drop
        ...
        X= rand_LogN_Norm_Correl(MM, VV, mg, vg, correl ,r); %draw randomly the lag time and growth rate correlated as in the experiment.
        tlag = X(:,1); 
        grate = X(:,2);
...
alpha = 0.1;
yCorrel = tsync; % your mean vector;
xCorrel = log(inocula);
std_dev = tstdsync;
curve1 = yCorrel + std_dev;
curve2 = yCorrel - std_dev;
x2 = [x, fliplr(x)];
inBetween = [curve1, fliplr(curve2)];
fill(x2, inBetween, 'b','FaceAlpha',alpha);
plot(x, yCorrel, '.-b', 'LineWidth', 5,'MarkerSize',20,'DisplayName', 'Leader');
...
inBetween = [curve1, fliplr(curve2)];
fill(x2, inBetween, 'b','FaceAlpha',alpha);
plot(x, y, '.-.b', 'LineWidth', 5,'MarkerSize',20,'DisplayName', 'Stat');
...
% inBetween = [curve1, fliplr(curve2)];
% fill(x2, inBetween, 'b','FaceAlpha',alpha);
% plot(x, y, ':b', 'LineWidth', 5,'MarkerSize',20,'DisplayName','No Noise');

clr = {'b' 'r' 'g' 'k'};
for i = 1:size(lag,2)
    h = errorbar(log(N0(:,i)), lag(:,i), slag(:,i),'d', 'MarkerSize',20, 'color', clr{i+1}, 'LineWidth',3, 'CapSize', 40);
end
